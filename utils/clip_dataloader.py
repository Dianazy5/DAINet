...
class bert_data():
    def __init__(self, max_len, batch_size, vocab_file, category_dict, num_workers=2, clip_model_name="ViT-B-16", clip_download_root='./'):
        ...
        self.bert_tokenizer = BertTokenizer.from_pretrained(self.vocab_file)

        self.clip_tokenizer = clip.tokenize

        if clip_model_name == "ViT-B-16":
            self.clip_context_length = 52
        elif "RN50" in clip_model_name:
            self.clip_context_length = 77
        else:
            raise ValueError(f"Unsupported CLIP model for context length: {clip_model_name}")
    ...
    def load_data(self, data_path, image_pkl_path, clip_image_pkl_path, shuffle, text_only=False):
        ...
        if not isinstance(all_images, torch.Tensor):
            if not isinstance(all_images, list) or not all_images:
                raise TypeError(f"Image PKL must contain a tensor or a non-empty tensor list: {image_pkl_path}")
        ...
        if not isinstance(all_clip_images, torch.Tensor):
            if not isinstance(all_clip_images, list) or not all_clip_images:
                raise TypeError(f"CLIP image PKL must contain a tensor or a non-empty tensor list: {clip_image_pkl_path}")
            all_clip_images = torch.stack(all_clip_images)
