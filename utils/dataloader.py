...
import pickle
from utils.domain_labels import UNCERTAIN_CATEGORY, WEIBO_CATEGORY_DICT, category_to_id
import pandas as pd
import numpy as np
...
def df_filter(df_data):
    df_data = df_data[df_data['category'] != UNCERTAIN_CATEGORY]
    return df_data
...
class bert_data():
    ...
    def load_data(self,path,ttv,shuffle,text_only = False):
        ...
        category = torch.tensor(self.data['category'].astype('object').apply(lambda c: category_to_id(c, self.category_dict)).to_numpy())
