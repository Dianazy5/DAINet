...
class MultiDomainPLEFENDModel(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(mlp_dims, (list, tuple)) or len(mlp_dims) == 0:
            raise ValueError("mlp_dims 必须是非空列表/元组，且第一个元素作为 hidden_dim。")
        self.hidden_dim = int(mlp_dims[0])
        if self.hidden_dim <= 1:
            raise ValueError("hidden_dim 必须大于 1。")
        self.domain_num = int(domain_num)
        self.num_scales = int(num_scales)
        if self.domain_num <= 0:
            raise ValueError("domain_num 必须大于 0。")
        if self.num_scales <= 0:
            raise ValueError("num_scales 必须大于 0。")
    ...
    def _encode_clip_global(self, clip_image_input, clip_text_input):
        ...
        if clip_image_input is None or clip_text_input is None:
            raise ValueError("缺少 clip_image 或 clip_text，无法计算全局特征。")
    ...
    def forward(self, **kwargs):
        ...
        if category is None:
            ...
            domain_ids = category.long().clamp(min=0, max=self.domain_num - 1)
...
class Trainer:
    ...
    def __init__(
        ...
        if not isinstance(self.category_dict, dict):
            raise TypeError("category_dict 必须是 dict，用于确定领域类别数量。")
        if self.epoches <= 0:
            raise ValueError("epoches 必须大于 0。")
        ...
        for epoch in range(self.epoches):
            ...
            for batch in train_iter:
                ...
                if batch_data is None:
                    raise RuntimeError("clipdata2gpu 返回 None，无法继续训练。")
                if "label" not in batch_data:
                    raise KeyError("batch_data 中缺少 label 字段。")
                ...
                if not torch.isfinite(total_loss):
                    raise FloatingPointError("total_loss 出现 NaN/Inf。")
    ...
    def test(self, dataloader):
        ...
        if dataloader is None:
            raise ValueError("test dataloader 不能为空。")
        self.model.eval()
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader):
                batch_data = clipdata2gpu(batch, self.use_cuda)
                if batch_data is None:
                    raise RuntimeError("clipdata2gpu 返回 None，无法继续评估。")
                if "label" not in batch_data:
                    raise KeyError("batch_data 中缺少 label 字段。")
        ...
        if all_conflict_weights:
            ...
            if len(all_weight_categories) == torch.cat(all_conflict_weights, dim=0).size(0):
                weights = torch.cat(all_conflict_weights, dim=0)
                categories = torch.tensor(all_weight_categories, dtype=torch.long)
